...
class PrMAnalysisEstimateDiff(PrMAnalysisBase):
    '''
    A class to perform quick PrM analysis based on the difference between cathode and anode
    '''
    ...
    def _pre_process(self, smooth=True, n=10):
        ...
        self._wf_diff = self._raw_wf_c - self._raw_wf_a
        self._wf_x = self._raw_wf_x

        if smooth:
            self._wf_diff = np.convolve(self._wf_diff, np.ones(n)/n, mode='valid')
            self._wf_x = self._wf_x[int(n/2):-int(n/2)+1]

        if self._debug: print('Preprocessed.')
    ...
    def _estimate_baseline(self):
        ...
        Args:
            ...
            baseline_range_a (list): Range used to estimate baseline for anode
        '''
        self._baseline_c = np.mean(self._wf_diff[self._baseline_range_c])
        self._baseline_rms_c = np.std(self._wf_diff[self._baseline_range_c])

        self._baseline_a = np.mean(self._wf_diff[self._baseline_range_a])
        self._baseline_rms_a = np.std(self._wf_diff[self._baseline_range_a])
    ...
    def _estimate_deltat(self):
        ...
        # Cathode
        # try:
        tmp_wvf = self._wf_diff[self._signal_range_c[0]:self._signal_range_c[1]]
        min_idx = np.argmin(tmp_wvf)
        self._max_c = np.min(tmp_wvf)
        if self._debug: print('min_idx', min_idx)
        if self._debug: print('Cathode MIN', self._max_c)
        ...
        # Anode
        # try:
        tmp_wvf = self._wf_diff[self._signal_range_a[0]:self._signal_range_a[1]]
        min_idx = np.argmin(tmp_wvf)
        self._max_a = np.min(tmp_wvf)
        if self._debug: print('Anode MAX', self._max_a)
        selected = tmp_wvf[0:min_idx]
        ...
        start_idx = np.argwhere((selected-self._baseline_a)>th)[-1]
        start = self._wf_x[start_idx + self._signal_range_a[0]]
        end = self._wf_x[min_idx + self._signal_range_a[0]]
        if self._debug: print('Anode: start', start, 'end', end)
        self._time_start_a = start[0]
        self._time_end_a = end
        self._deltat_a = end - start[0]
